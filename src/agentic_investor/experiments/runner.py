...
def _arm_env(arm_alpaca_account: str, arm_db_path: Path) -> dict[str, str]:
    """Return env overrides for one arm's subprocess.

    Preserves existing env, only overrides DATABASE_URL so the arm's
    LoopState + orders + snapshots write to their own SQLite file.
...
def run_experiment(
    ...
    *,
    ...
    for arm in experiment.arms:
        ...
        env = _arm_env(arm.alpaca_account, arm_db)
        cmd = [
        ...
        t.start()
        threads.append(t)
        # Stagger arm launch a few seconds so their news streamer opens
        # don't collide on identical timing.
        time.sleep(2)
    if dry_run_launch:
        return 0
